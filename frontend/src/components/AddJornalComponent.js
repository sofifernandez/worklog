...
export const AddJornalComponent = () => {
    ...
    useEffect(() => {
        if (id) { // es necesario verificar, porque cuando id = undefined el efecto se ejecuta
    ...
    useEffect(() => {
        const fetchObra = async () => {
            if (personaRolLoggeado.personaRol.rol.rol === 'JEFE_OBRA') {
                try {
                    const obraData = await JefeObraService.getObraByJefeId(personaRolLoggeado.id);
                    if (obraData) {
    ...
    if (personaRolLoggeado.personaRol.rol.rol === 'JEFE_OBRA' && !obra) {
        return <h2 className='text-center mt-5'>No puedes agregar jornales si no tienes obra asignada.</h2>;
    }
        ...
        <div className='mt-5 row justify-content-center col-11'>
          <div className='card cardForm col-lg-10'>
            ...
            <div className='card-body'>
              <form>
                {persona && (
                  <h6 className='px-0'>Persona Seleccionada: <span className='px-0 text-secondary'>{persona.nombre} {persona.apellido}</span></h6>
                )}
                {!personaTemp && (
                ...
                { !persona && personaTemp && (
                  <div className='row justify-content-center'>
                    ...
                    <button className="btn btn-success py-2 m-3 col-4" type="submit" onClick={(e) => seleccionarPersona(personaTemp)}>Seleccionar</button>
                    <button className="btn btn-danger py-2 m-3 col-4" type="submit" onClick={cancelar}>Atrás</button>
                  </div>
                )}
                {persona && obra && (
                  <h6 className='px-0'>Obra Seleccionada: <span className='px-0 text-secondary'>{obra.nombre}</span></h6>
                )}
                {persona && !obraTemp && !obra && (
                  <div className='row justify-content-center'>
                    <ContainerBuscadorByBPSComponent onObraFound={handleObraFound} onCancelar={volverBuscarPersona} />
                  </div>
                )}
                { persona && !obra && obraTemp && (
                  <div className='row row justify-content-center'>
                    <DatoObraComponent obra={obraTemp}/>
                    <button className="btn btn-success py-2 m-3 col-4" type="submit" onClick={(e) => seleccionarObra(obraTemp)}>Seleccionar</button>
                    <button className="btn btn-danger py-2 m-3 col-4" type="submit" onClick={cancelar}>Atrás</button>
                  </div>
                )}
                {persona && obra && !fechaJornal && (
                  <h5 className='px-0 text-primary'><br />Seleccionar Fecha de Jornal</h5>
                )}
                {persona && obra && fechaJornal && (
                  <h6 className='px-0'>Fecha Seleccionada: <span className='px-0 text-secondary'>{fechaJornal}</span></h6>
                )}
                {persona && obra && !fechaJornal && (
                  <div className='form-group'>
                    ...
                    /><br />
                    <button className="btn btn-success py-2 m-3 col-4" type="submit" onClick={(e) => setFechaJornal(fechaJornalTemp)}>Seleccionar</button>
                    <button className="btn btn-danger py-2 m-3 col-4" type="submit" onClick={cancelar}>Atrás</button>
                  </div>
                )}
                {persona && obra && fechaJornal && !horaComienzo && (
                  <h5 className='px-0 text-primary'><br />Seleccionar Hora de Entrada</h5>
                )}
                {persona && obra && fechaJornal && horaComienzo && (
                  <h6 className='px-0'>Hora de Entrada: <span className='px-0 text-secondary'>{horaComienzoDef}</span></h6>
                )}
                {persona && obra && fechaJornal && !horaComienzo && (
                  <div className='form-group row row justify-content-center'>
                    ...
                    <button className="btn btn-success py-2 m-3 col-4" type="submit" onClick={(e) => handleHoraComienzo(horaComienzoTemp)}>Seleccionar</button>
                    <button className="btn btn-danger py-2 m-3 col-4" type="submit" onClick={cancelar}>Atrás</button>
                  </div>
                )}
                {persona && obra && fechaJornal && horaComienzo && !horaFin && (
                  <h5 className='px-0 text-primary'><br />Seleccionar Hora de Salida</h5>
                )}
                {persona && obra && fechaJornal && horaComienzo && horaFin && (
                  <h6 className='px-0'>Hora de Salida: <span className='px-0 text-secondary'>{horaFinDef}</span></h6>
                )}
                {persona && obra && fechaJornal && horaComienzo && !horaFin && (
                  <div className='form-group row row justify-content-center'>
                    <div>
                    ...
                    <button className="btn btn-success py-2 m-3 col-4" type="submit" onClick={(e) => handleHoraFin(horaFinTemp)}>Seleccionar</button>
                    <button className="btn btn-danger py-2 m-3 col-4" type="submit" onClick={cancelar}>Atrás</button>
                  </div>
                )}
                {persona && obra && fechaJornal && horaComienzo && horaFin && !tipoJornal && (
                  <h5 className='px-0 text-primary'><br />Seleccionar Tipo de Jornal</h5>
                )}
                {persona && obra && fechaJornal && horaComienzo && horaFin && tipoJornal && (
                  <h6 className='px-0'>Tipo de Jornal: <span className='px-0 text-secondary'>{tipoJornalTexto(tipoJornal)}</span></h6>
                )}
                {persona && obra && fechaJornal && horaComienzo && horaFin && !tipoJornal && (
                  <div className='form-group row row justify-content-center'> 
                    ...
                    <button className="btn btn-success py-2 m-3 col-4" type="submit" onClick={(e) => setTipoJornal(tipoJornalTemp)}>Seleccionar</button>
                    <button className="btn btn-danger py-2 m-3 col-4" type="submit" onClick={cancelar}>Atrás</button>
                  </div>
                )}
                {persona && obra && fechaJornal && horaComienzo && horaFin && tipoJornal && (
                  <div className='form-group row row justify-content-center'> 
                    ...
                    <button className="btn btn-info py-2 m-3 col-3" type="submit" onClick={cancelar}>Limpiar</button>
                    <button className="btn btn-danger py-2 m-3 col-3" type="submit" onClick={volver}>Atrás</button>
                  </div>
                )}
              </form>
            </div>
